...
def locations(request, city):
  ...
  if title:
    try:
      locations_obj = loads(movies_json)
    except ValueError as e:
      print "Error: Can not decode json: %s\n%s" % (movies_json, e)
    title_locations = [ location for location in locations_obj if location['title'].lower() == title.lower() ]
...
def titles(request, city):
  ...
  try:
    ...
    print "Error: Can not decode json: %s\n%s" % (movies_json, e)
  titles_set = set([ location['title'] for location in locations_obj ])  # De-dupe titles
